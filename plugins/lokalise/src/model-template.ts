...
export const getTranslationModelTemplate = (
  ...
  fields: [
    {
      ...
      type: 'text',
      required: false,
      helperText: 'Lokalise task description',
    },
    {
      '@type': '@builder.io/core:Field',
      name: 'entries',
  ...
  helperText: 'Lokalise Translation Jobs',
  ...
  hooks: {
    validate: `async function run() {
      if (contentModel.published === 'published') {
          return {
              level: 'error',
              message: 'Job was already sent to Lokalise, please duplicate to create a new job'
          }
      }
      if (!contentModel.data.get('jobDetails')?.get('project')) {
        return {
            level: 'error',
            message: 'Please choose a Lokalise project for this job'
        }
      }
      const jobDetails = contentModel.data.get('jobDetails');
      if (!jobDetails?.get('targetLocales')?.length) {
        return {
            level: 'error',
            message: 'Please choose at least one target locale for this job'
        }
      }
      const hasAssignees = jobDetails.get('targetLocales').every(locale => locale.assignees?.length > 0);
      if (!hasAssignees) {
        return {
              level: 'error',
              message: 'Please assign contributors to each target locale for this job'
        }
      }
  ...
  webhooks: [
    {
      ...
      url: `${appState.config.apiRoot()}/api/v1/lokalise/job-publish-hook?apiKey=${apiKey}&pluginId=${pluginId}`,
